 ...
 *
 * Flow:
 * 1. Open the authorize URL as a Zendesk admin
 * 2. Click Allow — Zendesk redirects here with ?code=
 * 3. Copy the code into your token-exchange script (or set env vars below)
 *
 * Optional auto-exchange (server-side only) when these Vercel env vars are set:
 *   ZENDESK_SUBDOMAIN, ZENDESK_OAUTH_CLIENT_ID, ZENDESK_OAUTH_CLIENT_SECRET
 *   ZENDESK_OAUTH_REDIRECT_URI (must match Zendesk exactly)
 */
module.exports = async function handler(req, res) {
  ...
  // If OAuth app credentials are configured, exchange code → tokens here.
  if (subdomain && clientId && clientSecret) {
    try {
      ...
      // Tokens are sensitive — only return them when explicitly enabled for setup.
  ...
  // Manual path: no env credentials — return the code for your local exchange script.
  return res.status(200).json({
    message:
